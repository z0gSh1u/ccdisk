...
- **💿 盘片管理** - 在精选的工作环境配置（编程、数据、写作）之间切换，每个配置具有不同的技能、MCP 服务器和系统提示词
...
**盘片（Disk）**是 CCDisk 创新的工作环境系统，解决了"上下文污染"问题——当加载过多工具时会让 AI 不堪重负。盘片是一个自包含的工作环境配置文件，为特定场景定义 AI 助手的完整能力集。

#### 什么是盘片？

每个盘片包含：

- **技能** - 为 AI 精选的基于 Markdown 的行为模式
- **MCP 服务器** - 扩展 Claude 能力的模型上下文协议服务器
- **命令** - 用于任务自动化的可执行脚本
- **系统提示词** - 定义 AI 角色、风格和约束的行为指令
- **模型偏好** - 首选的 Claude 模型标识符（例如 `claude-sonnet-4-20250514`）

#### 盘片工作原理

- **完全替换** - 切换盘片时完全替换活动的技能/MCP/命令；没有叠加或合并
- **全局切换** - 盘片选择是全局设置，影响所有新会话
- **会话标记** - 会话会标记其创建时使用的盘片，但所有会话在侧边栏中仍然可见并带有标签
- **池 + 引用模型** - 技能、命令和 MCP 配置存储在中央池中（`~/.ccdisk/`）；盘片仅通过 ID 引用它们以避免重复

#### 内置盘片

CCDisk 附带四个预配置的盘片：

1. **默认（Default）** - 通用模式，使用 `~/.claude/` 中现有的全局配置（零迁移成本）
2. **编程（Coding）** - 针对软件开发优化，包含前端设计、React 最佳实践、MCP 构建器和 Playwright 技能
3. **数据（Data）** - 专注于数据处理和分析，包含 CSV 汇总器、Excel 和 PostgreSQL 技能
4. **写作（Writing）** - 专业文档创作，包含 DOCX、PDF、PPTX 和 EPUB 技能
...
- 复制现有盘片作为起始模板
- 从中央池中选择技能、命令和 MCP 服务器
- 定义自定义系统提示词以塑造 AI 行为
- 为特定任务选择首选模型
- 通过直观的界面在**设置 → 盘片管理**中管理盘片
...
├── skills/             # 全局技能池
├── commands/           # 全局命令池
└── mcp-servers.json    # 全局 MCP 服务器池
```

盘片让你能够在不同的 AI "模式"之间即时切换——从专注的编程助手到数据分析师再到专业作家——只需一次点击，确保 Claude 始终拥有适合当前任务的恰当工具。

...
export function registerSkillsHandlers(skillsService: SkillsService) {
  ...
  // Get skill
  ipcMain.handle(IPC_CHANNELS.SKILLS_GET, async (_event, name: string, scope: 'global' | 'workspace') => {
    try {
  ...
  // Create skill
  ipcMain.handle(IPC_CHANNELS.SKILLS_CREATE, async (_event, name: string, content: string, scope: 'global' | 'workspace') => {
    try {
  ...
  // Update skill
  ipcMain.handle(IPC_CHANNELS.SKILLS_UPDATE, async (_event, name: string, content: string, scope: 'global' | 'workspace') => {
    try {
  ...
  // Delete skill
  ipcMain.handle(IPC_CHANNELS.SKILLS_DELETE, async (_event, name: string, scope: 'global' | 'workspace') => {
    try {

        ...
        {items.map((item, index) => (
          <button
            ...
            className={`w-full px-3 py-2 text-left flex items-start gap-3 transition-colors ${
              index === selectedIndex ? 'bg-accent bg-opacity-10 text-accent' : 'hover:bg-gray-50 text-text-primary'
            }`}
          >
            <div className="mt-0.5 flex-shrink-0">{getIcon(item)}</div>
            <div className="flex-1 min-w-0">
              <div className="font-medium truncate">{item.label}</div>
              {item.description && <div className="text-xs text-text-tertiary truncate mt-0.5">{item.description}</div>}
            </div>
          </button>
        ))}
      </div>
